...
function FAQSection() {
  const faqs = [
    ...
    { q: '얼굴을 공개해야 하나요?', a: '아니요, 얼굴 공개 없이 운영할 수 있는 채널 유형도 강의에서 다룹니다. Part 1에서 얼굴 노출 여부에 따른 채널 구조와 장단점을 비교해드리고, 본인에게 맞는 방향을 고르실 수 있게 설계했습니다.' },
    { q: '촬영 장비나 편집 프로그램이 없어도 되나요?', a: '괜찮습니다. 고가 장비 없이 스마트폰 + 무료 편집 프로그램만으로 시작할 수 있는 워크플로우를 Part 5에서 실전 예시와 함께 보여드립니다. 편집 시간을 절반으로 줄이는 루틴도 포함됩니다.' },
    { q: '수강 기한은 언제까지인가요?', a: '올인원 패스는 결제일로부터 4개월간 강의와 프로그램을 모두 이용할 수 있습니다. 기간 내에는 횟수 제한 없이 반복 수강 가능합니다.' },
    { q: '강의는 어떻게 제공되나요?', a: 'VOD 온라인 스트리밍 방식입니다. 로그인 후 대시보드의 강의 플레이어에서 바로 재생할 수 있고, DRM 보호 영상이라 다운로드는 제공되지 않습니다. PC/모바일 모두 시청 가능합니다.' },

    // 프로그램 (FlowSpot)
    { q: '크레딧은 어떻게 사용되나요?', a: '스크립트 1회 생성 = 10크레딧 차감 방식입니다. 올인원 패스는 결제 즉시 400cr 지급되고, 이후 매달 400cr씩 총 4회(1,600cr) 지급됩니다. 1달에 약 40회 생성 분량입니다.' },
    { q: '크레딧이 부족하면 어떻게 되나요?', a: '강의와 월간 트렌드 데이터는 계속 이용 가능하며, 스크립트 생성만 제한됩니다. 대시보드에서 추가 크레딧을 따로 구매할 수 있습니다.' },
    { q: '어떤 주제든 생성할 수 있나요?', a: '현재는 "지식 정보형"과 "썰/설렘형" 두 가지 니치에 최적화되어 있습니다. 두 카테고리에 해당하는 주제라면 어떤 소재든 넣어 생성할 수 있고, 동물/역사/여행 등 다른 니치는 순차적으로 추가될 예정입니다.' },
    { q: 'AI로 만든 스크립트의 저작권은 누구에게 있나요?', a: '생성된 스크립트는 사용자가 자유롭게 활용하실 수 있습니다. 상업적 이용, 수정, 재배포 모두 제한 없이 가능합니다. 다만 AI 생성물 특성상 타인의 저작물과 우연히 유사할 수 있으니 최종 검수는 직접 해주세요.' },
    { q: '생성된 스크립트가 마음에 안 들면요?', a: '같은 소재로 다시 생성하면 매번 다른 훅/바디 조합으로 나옵니다. 강의 Part 3에서 생성 결과를 어떻게 재활용하고 다듬는지, 3개 중 어떤 걸 골라야 하는지 판단 기준도 같이 알려드립니다.' },
    ...
    { q: '환불 되나요?', a: '결제일로부터 28일 이내 3구간 환불이 가능합니다. 1~7일 이내 & 5강 미만 수강 & 크레딧 미사용 시 전액 환불, 8~14일은 결제금의 2/3에서 위약금 10% 공제, 15~28일은 1/2에서 위약금 10% 공제 후 환불됩니다. 29일 경과 또는 5강 이상 수강/크레딧 사용 시에는 환불이 제한됩니다. 자세한 조건과 예시는 "/refund" 페이지를 확인해 주세요.' },
    { q: '크레딧만 따로 구매할 수 있나요?', a: '네, 올인원 결제 이후 대시보드에서 크레딧 단독 충전이 가능합니다. 다만 강의와 프로그램까지 포함된 올인원이 크레딧만 기준으로 환산해도 더 저렴하게 설계되어 있습니다.' },
    { q: '결제 수단은 뭐가 있나요?', a: '신용/체크카드, 계좌이체, 토스페이, 카카오페이, 네이버페이 등을 지원합니다. 결제는 토스페이먼츠(PG)를 통해 안전하게 처리됩니다.' },
    { q: '세금계산서나 현금영수증 발행되나요?', a: '개인은 결제 후 현금영수증 발행이 가능하고, 사업자는 세금계산서 발행을 지원합니다. 발행이 필요하신 경우 hmys0205hmys@gmail.com 으로 결제 정보와 함께 요청해 주세요.' },
    { q: '해외에서도 결제/수강 가능한가요?', a: '해외에서도 한국 발급 카드 또는 해외 카드로 결제 가능합니다. 강의와 프로그램 모두 웹 기반이라 국가 제한 없이 접속 가능합니다. 단 고객 응대는 한국어로만 제공됩니다.' },

    // 지원
    { q: '문의는 어디로 하면 되나요?', a: 'hmys0205hmys@gmail.com 으로 메일 주시면 영업일 기준 24~48시간 내 답변드립니다. 결제/환불, 수강, 프로그램 오류 모두 같은 주소로 문의 가능합니다.' },
    { q: '계정 공유가 가능한가요?', a: '계정 공유는 금지됩니다. 1인 1계정 원칙이며, 비정상적인 동시 접속이 감지될 경우 이용이 제한될 수 있습니다. 자료의 외부 공유/유출 또한 약관상 금지됩니다.' },
  ];
