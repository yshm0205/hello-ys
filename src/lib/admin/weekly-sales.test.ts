...
test("starts each reporting week on Saturday at midnight in Korea", () => {
  assert.equal(
...
test("builds a 12-week query range that includes the current week", () => {
  ...
  assert.equal(range.start.toISOString(), "2026-06-12T15:00:00.000Z");
  assert.equal(range.end.toISOString(), "2026-09-04T15:00:00.000Z");
});
